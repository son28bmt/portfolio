...
const resolveModelByProvider = (configData, provider = "chatgpt") => {
  ...
  if (normalizedProvider === "chatgpt") {
    return String(
      configData?.ai_model || MODEL_PROVIDER_DEFAULTS.chatgpt,
    ).trim();
  }
...
router.post("/config", protect, requireAdmin, async (req, res) => {
  ...
  try {
    const safeChatgptModel =
      modelChatgpt || model || MODEL_PROVIDER_DEFAULTS.chatgpt;
...
const aiLimiter = rateLimit({
  ...
  handler: (req, res) => {
    return res.status(429).json({
      error:
        "Hệ thống AI đang quá tải hoặc bạn đã sử dụng quá mức 30 lượt/giờ. Vui lòng đợi một lát rồi quay lại nhé!",
      reply:
        "Hệ thống AI đang quá tải hoặc bạn đã sử dụng quá mức 30 lượt/giờ. Vui lòng đợi một lát rồi quay lại nhé!",
    });
  },
...
router.post("/chat", aiLimiter, verifyTurnstile, async (req, res) => {
  ...
  try {
    ...
    const model = String(
      userModel || resolveModelByProvider(configData, normalizedModelProvider),
    ).trim();
    let systemPrompt =
      configData.ai_systemPrompt ||
      "Bạn là một trợ lý AI hữu ích. Bạn đang làm việc trên website nguyenquangson.id.vn. Nhiệm vụ của bạn là tư vấn nhiệt tình, thân thiện, trả lời ngắn gọn súc tích.";
    ...
    try {
      const [products, projects, blogs] = await Promise.all([
        ...
        Project.findAll({
          attributes: ["title", "description", "tech", "demo"],
          limit: 15,
          raw: true,
        }),
        Blog.findAll({
          attributes: ["title", "slug", "excerpt"],
          limit: 15,
          raw: true,
        }),
      ]);
      ...
      if (products.length > 0) {
        ragContext +=
          ...
          products
            .map(
              (p, i) =>
                `  ${i + 1}. ${p.name} - Giá: ${Number(p.price).toLocaleString()} VNĐ. Hỗ trợ: ${p.description}`,
            )
            .join("\n") +
          "\n\n";
      }
      ...
      if (projects.length > 0) {
        ragContext +=
          `* CÁC DỰ ÁN TRONG PORTFOLIO CỦA SƠN (Mục Dự Án):\n` +
          projects
            .map(
              (p, i) =>
                `  ${i + 1}. Tên project: ${p.title} | Stack: ${typeof p.tech === "string" ? p.tech : JSON.stringify(p.tech)} | Mô tả: ${p.description}`,
            )
            .join("\n") +
          "\n\n";
      }
      ...
      if (blogs.length > 0) {
        ragContext +=
          `* CÁC BÀI VIẾT NỔI BẬT (Mục Blog):\n` +
          blogs
            .map(
              (b, i) =>
                `  ${i + 1}. Bài viết: "${b.title}" - Tóm tắt: ${b.excerpt} - Link bài: /blog/${b.slug}`,
            )
            .join("\n") +
          "\n\n";
      }
    ...
    if (imageBase64) {
      userContent = [
        {
          type: "text",
          text: message || "Xin hãy mô tả hoặc phân tích hình ảnh này.",
        },
    ...
    if (response.data && response.data.choices && response.data.choices[0]) {
      ...
      res.status(500).json({
        reply:
          "API trả về kết quả không đúng định dạng. Vui lòng kiểm tra lại Base URL và Model.",
      });
    }
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!file && mode === "transcribe") {
      return res
        .status(400)
        .json({ message: "Vui lòng upload file video/audio" });
    }
    ...
    try {
      ...
      if (mode === "transcribe") {
        ...
        if (transBaseUrl.endsWith("/"))
          transBaseUrl = transBaseUrl.slice(0, -1);

        let apiUrl = transBaseUrl;
        if (
          !apiUrl.toLowerCase().includes("audio/transcriptions") &&
          !apiUrl.toLowerCase().includes("/transcriptions")
        ) {
          apiUrl = `${apiUrl}/audio/transcriptions`;
        }

        const isV98Proxy = /v98store\.com/i.test(transBaseUrl);
        const manualTranscribeModel = String(
          transcribeModel || configData.ai_transcribeModel || "",
        ).trim();
        const preferredTranscribeModel =
          manualTranscribeModel ||
          (isV98Proxy
            ? "whisper-1"
            : String(configData.ai_model || "gpt-4o-mini-transcribe").trim());

        const transcribeModelCandidates = Array.from(
          new Set(
            [
        ...
        for (const modelName of transcribeModelCandidates) {
          for (let attempt = 0; attempt < retryDelays.length; attempt += 1) {
            ...
            try {
              console.log(
                `Transcribe model attempt: ${modelName} (${attempt + 1}/${retryDelays.length})`,
              );
      ...
      if (mode === "translate") {
        if (!filePath || !filePath.toLowerCase().endsWith(".srt")) {
          return res
            .status(400)
            .json({ message: "Vui lòng upload file .srt để dịch" });
        }
            ...
            translation: {
              provider:
                normalizedTranslationProvider === "google"
                  ? "google"
                  : OPENAI_COMPATIBLE_PROVIDERS.has(
                ...
                translationModel ||
                  resolveModelByProvider(
                    ...
                    normalizeModelProvider(
                      translationModelProvider || normalizedTranslationProvider,
                      "chatgpt",
                    ),
                  ),
              ).trim(),
        ...
        const translatedSubs = Array.isArray(result)
          ? result
          : parseTranslatedFromString(result);
        if (!translatedSubs)
          throw new Error("Python tool trả về dữ liệu dịch không hợp lệ");
      ...
      if (mode === "render") {
        ...
        if (!subs)
          return res.status(400).json({ message: "Thiếu dữ liệu phụ đề" });
        ...
        const pythonInput = {
          ...
          output_path: path.join(
            workDir,
            `rendered_${Date.now()}.${videoPath ? "mp4" : "mp3"}`,
          ),
          config: {
            tts: {
      ...
      if (providerStatus === 429) {
        return res
          .status(429)
          .json({
            message: "API hết quota hoặc bị giới hạn.",
            error: finalProviderMsg,
          });
      }
      if (providerStatus) {
        return res
          ...
          .json({
            message: "Lỗi từ nhà cung cấp AI.",
            error: finalProviderMsg,
          });
      }
