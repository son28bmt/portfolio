    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-white/40">Quản lý danh sách bài viết và điều hướng qua trang tự động AI.</p>
        </div>
        <div className="flex items-center gap-2">
          <Link
            to="/blog/automation"
            className="px-4 py-3 bg-primary/20 border border-primary/30 text-primary rounded-2xl font-bold flex items-center gap-2 hover:scale-[1.02] transition-all"
          >
            <Bot className="w-5 h-5" /> Tự động AI
          </Link>
          <Link
            to="/blog/add"
                    ...
                    <tr key={post.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-8 py-6">
                        <div className="flex gap-1 flex-wrap">
                          {tags.map((t, i) => (
                            <span key={`${post.id}-${i}`} className="text-[10px] text-secondary bg-secondary/10 px-2 py-0.5 rounded-md font-bold">
                              {t}
                            </span>
                          ))}
                        </div>
