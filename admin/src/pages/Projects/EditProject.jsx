...
const EditProject = () => {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const techArray = toArray(data?.tech);
        const imageArray = toArray(data?.images);
        const cover = (data?.image || imageArray[0] || '').trim();
        const mergedImages = [...new Set([cover, ...imageArray].filter(Boolean))];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const galleryImages = Array.isArray(formData.images) ? formData.images.filter(Boolean) : [];
      ...
      alert('Lỗi khi cập nhật dự án: ' + (err.response?.data?.message || err.message));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-white/40">Cập nhật thông tin chi tiết cho dự án của bạn.</p>
        </div>
      </div>
